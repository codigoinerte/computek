    ...
    <div class="container">
      <div class="row">
		...
		<div class="toolbar">	
			...
			<?php 
			if($numeroRegistros > 8):
				$_alias_registro = $_alias."_";
				echo '';
				echo '<div class="pager"><div class="pages"><ul class="pagination ">';
				if($pagina>1) 
				{ 
				   echo '<li><a class="btn-lg" href="'.URL_WEB.$_alias_registro.($pagina-1).'" aria-label="Previous"><span aria-hidden="true">&laquo;</span></a></li>';
				} 
				for($i=$inicio;$i<=$final;$i++) 
				{ 
				   if($i==$pagina) 
				   { 
					  echo '<li class="active"><a class="btn-lg" href="#">'.$i.'<span class="sr-only">(current)</span></a></li>';
				   }
				   else
				   { 
					  echo '<li><a class="btn-lg" href="'.URL_WEB.$_alias_registro.$i.'">'.$i.'</a></li>';
				   } 
				} 
				if($pagina<$numPags) 
				{ 
				   echo '<li><a class="btn-lg" href="'.URL_WEB.$_alias_registro.($pagina+1).'" aria-label="Next"><span aria-hidden="true">&raquo;</span></a></li>';
				} 
				echo '</ul></div></div>';
				echo '';
			endif;   
