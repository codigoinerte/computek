...
switch ($get_action) {
    case 'new':    				
		...
		$array_id = $datos_registro->insertar_registro($_item_nombre, $_item_url, "", "", $_item_descripcion, $_item_orden, 3, $_item_estado, 1, 0, $fecha_creacion, $fecha_creacion);
    ...
    case 'edit':
		...
		if(count($_item_galeria) > 0)
		{
			foreach($_item_galeria as $item)
			{
				...
				$_galeria_url = isset($item["url"])?$item["url"]:'';
				$_galeria_orden = isset($item["orden"])?$item["orden"]:0;
				$_galeria_destacado = isset($item["destacado"])?$item["destacado"]:0;				
				$datos_registro->actualizar_registro($_galeria_id, $_galeria_nombre, $_galeria_url,"","","",$_galeria_orden,0,$_galeria_destacado);
			}
		}
